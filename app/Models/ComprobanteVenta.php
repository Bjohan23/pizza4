...
<<<<<<< HEAD
    ...
    {
=======
    public function getVentas($pagina, $limite)
    {
...
=======
    ...
    {
        $this->db->query('SELECT COUNT(*) AS total FROM comprobanteventa');
        return $this->db->single()['total'];
    }

    public function getVentasPorFecha($fecha, $pagina, $limite)
    {
        $inicio = ($pagina - 1) * $limite;
        ...
        JOIN 
            productos ON detallespedido.producto_id = productos.id
        WHERE
            DATE(comprobanteventa.fecha) = :fecha
        GROUP BY 
            comprobanteventa.id,
            comprobanteventa.pedido_id,
            comprobanteventa.fecha,
    ...
    public function getTotalVentasPorFecha($fecha)
    {
        $this->db->query('SELECT COUNT(*) AS total FROM comprobanteventa WHERE DATE(fecha) = :fecha');
        $this->db->bind(':fecha', $fecha);
        return $this->db->single()['total'];
    }
